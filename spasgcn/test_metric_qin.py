...
def NSS(saliency_map, fixation_map):
    ...
    f_map = np.array(fixation_map, copy=False) > 0
...
def getFrameFM(fixations, shape):
    fixationmap = np.zeros(shape, dtype=int)
    for subject in range(21):
        fixationmap[int(fixations[subject, 1]), int(fixations[subject, 0])] += 1
    return  fixationmap
...
def SphereToErp(final_height, sphere_img, point_theta):
    ...
    image = image.resize((final_height*2,final_height), Image.BICUBIC)
...
def cal_all_map(test_set, height, SAMPLING_TYPE, len_snippet, rate, label_rate, gaussian):
    H = height
    W = 2*height
    ...
    for vid in test_set:
        sal_root = f"/home/yq/Audio/Audio_visual/data/Qin/saliency_{gaussian}/{vid}"
        fix_root = f"/home/yq/Audio/Audio_visual/data/Qin/fixation/{vid}"
        fixmap_gt_all = np.load(fix_root+'/save.npy') #840*21*2
        fixmap_gt_all = -fixmap_gt_all+[180, 90]
        fixmap_gt_all = fixmap_gt_all*[(W-1)/360, (H-1)/180]
        for iFrame in range(int(len_snippet/2)*rate, int(len(os.listdir(sal_root))/2)-int(len_snippet/2)*rate, label_rate):
            fixmap_gt = getFrameFM(fixmap_gt_all[iFrame], [H, W])
            if SAMPLING_TYPE.split("_")[0] == "GICOPix":
...
def metric_qin(data_set, final_pth, predict_path, act, gaussian, len_snippet, rate, label_rate):  
    ...
    fixmap_all = cal_all_map(test_set, height, SAMPLING_TYPE, len_snippet, rate, label_rate, gaussian)
    ...
    with open(f"{predict_path}/metric.csv", "a") as saveFile:
        ...
        for vid in test_set:
            ...
            sal_root = f"/home/yq/Audio/Audio_visual/data/Qin/saliency_{gaussian}/{vid}" 
            fix_root = f"/home/yq/Audio/Audio_visual/data/Qin/fixation/{vid}"
            aem_root = f"/home/yq/Audio/Audio_visual/data/Qin/AEM/{vid}"

            fixmap_gt_all = np.load(fix_root+'/save.npy') #840*21*2
            fixmap_gt_all = -fixmap_gt_all+[180, 90]
            fixmap_gt_all = fixmap_gt_all*[(width-1)/360, (height-1)/180]
            ...
            SIMM_vid = 0
            salmap_idx_vid = 0
            for iFrame in range(int(len_snippet/2)*rate, len(os.listdir(sal_root))-int(len_snippet/2)*rate, label_rate): 
                    #predicted saliency map
                    print(salmap_pr.shape, salmap1_idx)
                    salmap_pred = salmap_pr[salmap1_idx] #10242
                    #salmap_pred = cv2.imread(os.path.join(aem_root, f'aem_{iFrame}.png')) #check AEM
                    #salmap_pred = cv2.cvtColor(salmap_pred, cv2.COLOR_BGR2GRAY) #check AEM
                    #salmap_pred = salmap_pred/255 #check AEM
                    #salmap_pred = cv2.imread(os.path.join(sal_root, f'{iFrame}.png')) #check GT
                    #salmap_pred = cv2.cvtColor(salmap_pred, cv2.COLOR_BGR2GRAY)  #check GT
                    salmap1_idx += 1
                    salmap_idx_vid += 1

                    
                    #label
                    salmap_gt = cv2.imread(os.path.join(sal_root, f'{iFrame}.png'))
                    salmap_gt = cv2.cvtColor(salmap_gt, cv2.COLOR_BGR2GRAY)

                    fixmap_gt = getFrameFM(fixmap_gt_all[iFrame], [height, width])


                    # Weight saliency maps vertically if specified
                    if SAMPLING_TYPE == "Sin":
                        salmap_pred = SphereToErp(height, salmap_pred, point_theta) #check GT
                    ...
                    print('NSS:%0.3f, CC:%0.3f'%(values[2], values[3]))
                    KL_vid += values[5]
                    AUC_S_vid += values[6]
                    AUC_B_vid += values[1]
                    AUC_J_vid += values[0]
